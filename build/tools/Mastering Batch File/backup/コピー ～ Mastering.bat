...
echo.
echo *** 出力ファイルが存在する場合バックアップを作成します。***
echo.
...
echo =================================================>>"%output_readme%"
echo マスタリング出力ファイルの説明>>"%output_readme%"
echo =================================================>>"%output_readme%"
...
if "%issecure%"=="YES" (
	echo "Secureアプリに限り、SDブート用のマスタリングが可能です"
	set /p user_input="SDブート用のマスタリングにしますか(Yes/No)> "
)
set sdboot_option=
if /i "%user_input%"=="yes" (
...
echo * %output_pro_short% : マスタリングパラメータ [逆マスタリングで必要になる] >>"%output_readme%"
...
echo.>>"%output_readme%"
echo * %output_cls_dir_short%      : CLS用のフォルダ構成 [このフォルダを丸ごとCLS用PCに持ち込んでCLS処理をかける]>>"%output_readme%"
echo   +- properties : CLSのパラメータ記述 [フォルダ内に存在しなかった場合 properties-v%tadversion% と一致するが、すでに存在していた場合には上書きされない]>>"%output_readme%"
echo   +- v%tadversion%\>>"%output_readme%"
echo      +- %output_tad_short% : マスタリング後のSRLをもとに maketad によって作成されたTAD>>"%output_readme%"
echo   +- properties-v%tadversion% : v%tadversion% の properties [前のバージョンを引き継がない場合には、このファイルを"properties"にリネームする]>>"%output_readme%"
...
if %isempty%=="NO" (
echo.>>"%output_readme%"
echo * %backup_dir_short% : バックアップフォルダ [マスタリング前のフォルダの中身がすべて入っている]>>"%output_readme%"
echo   +- ...>>"%output_readme%"
)
echo.>>"%output_readme%"
