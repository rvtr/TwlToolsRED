...
static BOOL RestoreFromSDCard4(void)
{
  if( mydata.user_settings_flag == TRUE ) {
    ...
    if( TRUE == MiyaRestoreTWLSettings( MyFile_GetUserSettingsFileName() ) ) {
      /*   国設定完了 LCFG_TSD_SetCountry( (LCFGTWLCountryCode)cfg_data.country ) */
      m_set_palette(tc[0], M_TEXT_COLOR_GREEN );
      mprintf("OK.\n");
      m_set_palette(tc[0], M_TEXT_COLOR_WHITE );
    }
...
static BOOL RestoreFromSDCard8(void)
{
  ...
  /*
    本来、EC downloadの後にNAM_ImportをやらないとLoadCertのSEA_Decryptでこける。
    理由はAESエンジンのスロットをNAM_Importでつぶしちゃうので、
    ここでCertの復号化だけやっとく。
  */
  (void)SetupEC_pre();
  /* ECDownload の前準備,終わり。*/
